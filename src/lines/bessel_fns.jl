...
let
    ...
    for ν in 0:4
        CairoMakie.lines!(ax, x, bessely.(ν, x), label = latexstring("Y_{$(ν)}(x)"), linewidth = 2)
    end
    axislegend(; position = :rb, nbanks = 2, framecolor = (:grey, 0.5))
    CairoMakie.ylims!(-1.8, 0.7)
    display(fig)
end;
...
let
    ...
    ax = Axis(fig[1, 1], xlabel = L"x", ylabel = L"Y_{\nu}(x)", ylabelsize = 22,
        ...
        xticksize = 10, ytickalign = 1, yticksize = 10, xlabelpadding = -10)
    for ν in 0:4
        CairoMakie.lines!(ax, x, sphericalbesselj.(ν, x), label = latexstring("Y_{$(ν)}(x)"), linewidth = 3)
    end
    axislegend(; position = :rb, nbanks = 2, framecolor = (:grey, 0.5))
    CairoMakie.ylims!(-.6, .6)
    display(fig)
end;
